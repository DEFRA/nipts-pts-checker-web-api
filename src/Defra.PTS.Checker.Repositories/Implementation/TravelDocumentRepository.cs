...
namespace Defra.PTS.Checker.Repositories.Implementation
{
    public class TravelDocumentRepository : Repository<entity.TravelDocument>, ITravelDocumentRepository
    {
        ...
        public async Task<IEnumerable<entity.TravelDocument>> GetByPetIdAsync(Guid petId)
        {
        ...
        public async Task<entity.TravelDocument?> GetTravelDocumentByApplicationIdAsync(Guid applicationId)
        {

            var travelDocument = await _context.TravelDocument
                .Where(td => td.ApplicationId == applicationId)
                .Select(td => new entity.TravelDocument
                {
                    Id = td.Id,
